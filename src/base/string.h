...
struct String {
    ...
    static String lit(const char* s) {
        assert_msg(s != nullptr, "String literal source must not be null!");
    ...
    static String fmt(Arena* arena, const char* format, ...)
        ...
        va_end(args_copy);
        assert_msg(size_needed >= 0, "String formatting failed!");
        ...
        assert_msg(!buffer_overflow, "String formatting size overflow!");
        u8* buffer = arena->push<u8>(buffer_size);
        int size_written = vsnprintf((char*)buffer, buffer_size, format, args);
        assert_msg(size_written == size_needed, "String formatting length mismatch!");
        result.str = buffer;
        va_end(args);
        return result;
    }
...
inline String str_concat(Arena* arena, String a, String b) {
    String result = {};
    bool size_overflow = u64_add_overflow(a.size, b.size, &result.size);
    assert_msg(!size_overflow, "String concatenation size overflow!");
    u64 buffer_size = 0;
    bool buffer_overflow =
        u64_add_overflow(result.size, 1ULL, &buffer_size);
    assert_msg(!buffer_overflow, "String concatenation size overflow!");
    u8* buffer = arena->push<u8>(buffer_size);
    if (a.size > 0) {
        assert_msg(a.str != nullptr, "Left string source must not be null!");
        memcpy(buffer, a.str, a.size);
    }
    if (b.size > 0) {
        assert_msg(b.str != nullptr, "Right string source must not be null!");
